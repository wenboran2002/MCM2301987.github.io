...
for i in range(0,350,3):
    max_time[i:i+3]=max(time[i:i+3].values)
    min_time[i:i+3]=min(time[i:i+3].values)
loss_list=[]
def cal(time,label_,color):
    ...
    iterrations=1000 if label_=='pre' else 500
    for e in range(iterrations):
        out = model(batch_x)
    ...
    print(data_x.shape)
    new_data_x = np.append(data_x.copy().flatten(),[0 for i in range(70)]).reshape(-1,1)
...
max_new_data=cal(max_time,'max_pre','blue')
min_new_data=cal(min_time,'min_pre','green')
new_data=cal(time,'pre','black')

print(min_new_data[-1],max_new_data[-1])
ax.fill_between(np.arange(len(max_time)+69),max_new_data.reshape(-1),min_new_data.reshape(-1),facecolor='red',alpha=0.1)
ax.set_title('number prediction')
ax.set_ylabel('number')
ax.set_xlabel('date')
# plt.plot(np.array(time), 'r', label='real', alpha=0.3)
plt.legend(loc='best')
plt.savefig('pre.svg',format='svg')
plt.show()
